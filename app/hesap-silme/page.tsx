...
export default function DeleteAccountPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email && !phone) {
      setError('Lutfen e-posta veya telefon numaranizi girin.')
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.message || 'Bir hata olustu. Lutfen daha sonra tekrar deneyin.')
      }
    } catch {
        ...
        <div className="max-w-[600px] mx-auto px-5">
          <ContentCard>
            <div className="text-center py-8">
              ...
              <h1 className="text-2xl font-bold mb-4 text-text">Talebiniz Alindi</h1>
              <p className="text-muted">
                Hesap silme talebiniz basariyla alindi. Talebiniz en kisa surede incelenecek ve
                size e-posta veya SMS ile bilgi verilecektir.
              </p>
              <p className="text-muted mt-4 text-sm">
                Islem suresi: 1-3 is gunu
              </p>
            </div>
          </ContentCard>
        </div>
  ...
  return (
    <main className="flex-1 py-16">
      <div className="max-w-[600px] mx-auto px-5">
        <ContentCard>
          <h1 className="text-3xl font-bold mb-2 text-text max-sm:text-2xl">Hesap Silme Talebi</h1>
          <p className="text-muted text-sm mb-8">
            Hesabinizi silmek istiyorsaniz asagidaki formu doldurun. Talebiniz incelendikten sonra
            hesabiniz ve tum verileriniz kalici olarak silinecektir.
          </p>

          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
            <p className="text-amber-800 text-sm">
              <strong>Onemli:</strong> Hesabiniz silindikten sonra tum verileriniz (profil bilgileri,
              basvurular, mesajlar) kalici olarak silinecek ve geri alinamayacaktir.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label htmlFor="reason" className="block text-sm font-medium text-text mb-2">
                Silme Sebebi <span className="text-muted font-normal">(Istege bagli)</span>
              </label>
              <textarea
                id="reason"
                value={reason}
          ...
          <p className="text-muted text-xs mt-6 text-center">
            Sorulariniz icin <a href="mailto:hello@hirovo.com" className="text-hirovo-teal hover:underline">hello@hirovo.com</a> adresinden bize ulasabilirsiniz.
          </p>
        </ContentCard>
      </div>
    </main>
