...
export default function AboutClient({ wpContent }: { wpContent: WPPage | null }) {
        ...
        <section className="mt-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              { value: "5K+", label: "activeUsers" },
              { value: "500+", label: "partnerCompanies" },
              { value: "2K+", label: "successfulMatches" },
            ].map((stat) => (
              <div key={stat.label} className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 text-center border border-white/20">
                <div className="text-4xl font-bold text-hirovo-teal mb-2">{stat.value}</div>
                <div className="text-white/70">{t(`stats.${stat.label}`)}</div>
              </div>
            ))}
          </div>
        </section>
